...
- **SHMÚ radar.** Keyless. `/api/shmu/radar` probes recent 5-minute slots of the ODIM_H5 `zmax` composite directly (deterministic filenames — no directory scraping), decodes with pure-JS jsfive, resamples Mercator rows to linear latitude, colorizes dBZ (`src/data/shmuRadarGrid.js`), and serves one PNG + bounds. TTL 5 min matching the product cadence (≤12 upstream fetches/hour, ~40 KB each), single-flight, disk cache (`.gev-cache/shmu-radar.*`), serve-stale with an explicit `stale` flag that the layer surfaces in its panel state. One 42 KB point-in-time HDF snapshot is committed as a decode-test fixture (`src/data/fixtures/`, © SHMÚ CC BY 4.0, never served to the app).
